...
class WebSocket:
    ...
    async def accept(
        self, subprotocol: str | None = None, headers: list[tuple[bytes, bytes]] | None = None
    ) -> None:
        """
    ...
    async def receive(self) -> dict[str, Any]:
        ...
        raise RuntimeError(
            f'Expected "websocket.receive" or "websocket.disconnect", got "{message_type}"'
        )
    ...
    async def receive_json(self, mode: str = "text") -> Any:
        ...
        try:
            return dict(orjson.loads(text))
        except orjson.JSONDecodeError as exc:
            raise RuntimeError(f"Malformed JSON data: {exc}") from exc
